...
export default function CreateTenantModal({
  ...
  // User creation form
  const [createUser, setCreateUser] = useState(true); // Default to creating user
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        console.log('Creating user for tenant:', newTenant.id, userData);
    ...
    <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50'>
      <div className='bg-gray-900/95 backdrop-blur-sm border border-white/10 rounded-2xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto'>
        <div className='flex items-center justify-between mb-6'>
          <h3 className='text-xl font-bold text-white'>Yeni Müşteri Oluştur</h3>
          <button
            onClick={onClose}
